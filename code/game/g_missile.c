...
void G_MissileImpact( gentity_t *ent, trace_t *trace ) {
	...
	if (other->takedamage)
	{
		if (other->client)
		{
			if (other->client->ps.powerups[RUNE_POWERUP_SLOT] == RUNE_REFLECTION)
			{

				// Reflection deals a certain amount of damage, then reflects the projectile and takes control of it
				...
				if (!Q_stricmp(ent->classname,"grenade"))
				{
					...
					G_Damage (other, ent, &g_entities[ent->r.ownerNum], velocity, NULL, (int)(ent->damage * (1-RUNE_REFLECTION_AMOUNT)), 0, ent->methodOfDeath);
					ent->damage *=  (RUNE_REFLECTION_AMOUNT);
					ent->splashDamage *=  (RUNE_REFLECTION_AMOUNT);
				...
				if (!Q_stricmp(ent->classname,"rocket") || !Q_stricmp(ent->classname,"plasma") || !Q_stricmp(ent->classname,"bfg"))
				{
					tent = G_TempEntity(trace->endpos , EV_MISSILE_HIT );
					tent->s.eventParm = DirToByte( trace->plane.normal );
	...
	if (other->takedamage) 
	{
		if (other->client->ps.powerups[RUNE_POWERUP_SLOT]==RUNE_PHASE)
		{
			if ((int)(random()*100)%100<RUNE_PHASE_AVOID_CHANCE)
			{
				ent->s.pos.trTime = level.time - MISSILE_PRESTEP_TIME;		// move a bit on the very first frame
				VectorCopy( trace->endpos, ent->s.pos.trBase );
				VectorMA(ent->s.pos.trBase,0.03,ent->s.pos.trDelta,ent->s.pos.trBase);
...
================
*/
void G_MissileImpact2( gentity_t *ent, trace_t *trace ) 
{
	gentity_t		*other;
	qboolean		hitClient = qfalse;
	...
	if (other->takedamage) 
	{
		if (other->client->ps.powerups[RUNE_POWERUP_SLOT]==RUNE_PHASE)
		{
			if ((int)(crandom()*100)%100<RUNE_PHASE_AVOID_CHANCE)
			{
				trap_LinkEntity( ent );
				return;
			}
		...
		if (!Q_stricmp(ent->classname,"weirdness_bubble"))
		{
			//Rune_Weird_Touch(ent,other,trace);
		}
	}
...
================
*/
void G_RunRuneEntity( gentity_t *ent ) 
{
	gentity_t		*other;
	qboolean		bounce = qfalse;
	vec3_t		origin;
	trace_t		tr;
	vec3_t	velocity;
	float	dot;
	...
	if ( tr.fraction != 1 ) 
	{
		...
		if ((!other->takedamage || bounce) && (ent->s.pos.trType == TR_GRAVITY || ent->s.eFlags & EF_BOUNCE))
		{
			// reflect the velocity on the trace plane
			...
			if ( ent->s.eFlags & EF_BOUNCE_HALF ) 
			{
				VectorScale( ent->s.pos.trDelta, 0.65, ent->s.pos.trDelta );
				// check for stop
				if ( tr.plane.normal[2] > 0.2 && VectorLength( ent->s.pos.trDelta ) < 40 ) 
				{
					ent->s.pos.trDelta[0] = crandom ()*400;
					ent->s.pos.trDelta[1] = crandom ()*400;
					ent->s.pos.trDelta[2] = random ()*500+100;
					return;
				}
			}
