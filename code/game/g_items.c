...
int Pickup_Health (gentity_t *ent, gentity_t *other) {
	...
	HealPlayer(other,quantity, goOverMax);
...
void Touch_Item (gentity_t *ent, gentity_t *other, trace_t *trace) {
	...
	// RQ3: unfortunately, we have to bypass this to implement the ability to block people from
	// picking up the rune they dropped when they died
	if ( !BG_CanItemBeGrabbed( g_gametype.integer, &ent->s, &other->client->ps ) && ent->item->giType != IT_RUNE ) {
		return;
	}
