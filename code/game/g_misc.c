...
void TeleportPlayer( gentity_t *player, vec3_t origin, vec3_t angles, qboolean dropFlags, qboolean pushForward ) 
{
	...
	if (dropFlags)
	{
		...
		if ( item ) 
		{
			...
			drop->count = ( player->client->ps.powerups[ i ] - level.time ) / 1000;
			if ( drop->count < 1 ) {
				drop->count = 1;
			}

			player->client->ps.powerups[ i ] = 0;
		}
	}
	...
	//RUNE: free your hook if you teleported
...
//	}
	...
	if( !destination ) {
		if( self->pos1[0] || self->pos1[1] || self->pos1[2] ) {
			TeleportPlayer( other, self->pos1, self->s.angles, qfalse, qtrue );
		}
		G_Damage( other, other, other, NULL, NULL, 100000, DAMAGE_NO_PROTECTION, MOD_TELEFRAG );
		return;
	}

	TeleportPlayer( other, destination->s.pos.trBase, destination->s.angles, qfalse, qtrue );
}
