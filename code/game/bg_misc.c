...
int MaxHealth(playerState_t *ps, qboolean includeOverMax);
int MaxArmor(playerState_t *ps, qboolean includeOverMax);
...
/*QUAKED item_rune (.3 .3 1) (-16 -16 -16) (16 16 16) suspended
*/
	{
		"item_rune",
		RUNE_ITEM_PICKUP_SOUND,
        { RUNE_ITEM_MODEL1, 
		RUNE_ITEM_MODEL_RING1,
		0, 0},
/* icon */		RUNE_ITEM_PICKUP_ICON,
/* pickup */	"Rune1",
		0,
		IT_RUNE,
		0,
/* precache */ "",
/* sounds */ RUNE_ITEM_PICKUP_SOUND2
	},
	{
		"item_rune",
		RUNE_ITEM_PICKUP_SOUND,
        { RUNE_ITEM_MODEL2, 
		RUNE_ITEM_MODEL_RING2,
		0, 0},
/* icon */		RUNE_ITEM_PICKUP_ICON,
/* pickup */	"Rune2",
		0,
...
/*QUAKED ice_trap (.3 .3 1) (-16 -16 -16) (16 16 16) suspended
*/
	{
		"ice_trap",
		"sound/items/protect.wav",
        { "models/runes/icetrap.md3", 
		0, 0, 0 },
/* icon */		"icons/envirosuit",
/* pickup */	"Ice Trap",
		0,
...
// RQ3:  functions that need to exists both client and server


// Encapsulate all health changes to one function for simplicity

// ps - playerstate info, so we can access their default max health (which includes handicapping info)
//		and their rune
//
// includeOverMax - set to qtrue if you want to include how far they can go over their max, thus
//					causing their health to count down
//						used mainly for health powerups that can get you beyond max (megahealth and tiny health)
int MaxHealth(playerState_t *ps, qboolean includeOverMax)
{
	int max=0;

	switch(ps->powerups[RUNE_POWERUP_SLOT])
	{
	case RUNE_GOOD_HEALTH:
		max = RUNE_GOOD_HEALTH_MAX_HEALTH;
		if (includeOverMax)
			max+=RUNE_HEALTH_OVER_MAX;
		break;

	case RUNE_REGENERATION:
		max = RUNE_REGEN_MAX_HEALTH;
		if (includeOverMax)
			max=ps->stats[STAT_MAX_HEALTH] * 2;
		break;
	case RUNE_ATHLETICS:
		max = RUNE_ATHLETICS_MAX_HEALTH;
		if (includeOverMax)
			max += RUNE_HEALTH_OVER_MAX;
		break;
/*
	case RUNE_BOXER:
		max = RUNE_BOXER_MAX_HEALTH;
		if (includeOverMax)
			max+=RUNE_MAX_OVER_HEALTH;
		break;
...
// Encapsulate all armor changes to one function for simplicity

// ps - playerstate info, so we can access their armor level and rune
//
// includeOverMax - not currently used as you can't go over armor, but left there for
...
int MaxArmor(playerState_t *ps, qboolean includeOverMax)
{
	...
	case RUNE_PLENTIFUL_ARMOR:
		max = RUNE_PLENTINFUL_ARMOR_MAX;
		break;
	case RUNE_REGENERATION:
		max = RUNE_REGEN_MAX_ARMOR;
		break;
	default:
		max = ps->stats[STAT_MAX_HEALTH] * 2;
		break;
