...
void CG_AddRefEntityWithPowerups( refEntity_t *ent, entityState_t *state, int team ) {
	...
	else if ( state->powerups & ( 1 << PW_PHASE ) ) 
	{
		ent->customShader = cgs.media.phaseShader;
		trap_R_AddRefEntityToScene( ent );
	} 
	else 
	{
		...
		{
			ent->customShader = cgs.media.reflectionShader;
			trap_R_AddRefEntityToScene( ent );
		}
...
/*	if ( cent->currentState.powerups & ( 1 << PW_DOUBLER ) ) {
	...
	// add the head
	//
	if (cent->currentState.powerups & (1 << PW_VENGEANCE))
	{
		// RQ3FIXME:  add the smaller blood trail
	}
	else
	{
