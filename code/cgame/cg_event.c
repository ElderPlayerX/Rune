...
static void CG_Obituary( entityState_t *ent ) {
	...
	if (attacker == target) {
		...
		case MOD_RUNE_PIERCING:
			...
			break;
		case MOD_RUNE_NEGATION:
			if (gender == GENDER_FEMALE)
				   message = "negated herself";
			else if (gender == GENDER_NEUTER)
				   message = "negated itself";
			else
				   message = "negated himself";
			break;
	...
	if ( attacker != ENTITYNUM_WORLD ) {
		...
		// RQ3: Kill messages
		case MOD_RUNE_VENGEANCE:
			if ( gender == GENDER_FEMALE )
				message = "got a taste of her own medicine from";
			else if ( gender == GENDER_NEUTER )
				message = "got a taste of its own medicine from";
			else
				message = "got a taste of his own medicine from";
			break;
		case MOD_RUNE_PIERCING:
			message = "was pierced by";
			break;
		case MOD_RUNE_REFLECTION:
			message = "was dazzled by";
			message2 = "'s reflection";
			break;
		case MOD_RUNE_REFLECTION_RAIL:
			message = "was killed when his rail bounced off";
			break;
		case MOD_RUNE_REFLECTION_BULLET:
			message = "was killed when his shot ricocheted off";
			break;
		case MOD_LIGHTNING_DISCHARGE:
		   message = "was discharged by";
		   break;
		case MOD_RUNE_IMPACT:
			message = "was bodychecked by";
			break;
		default:
...
//					break;
			...
			if ( es->number == cg.snap->ps.clientNum && item->giType != IT_RUNE) {
				CG_ItemPickup( index );
			}
		}
		break;
...
/*	case EV_POWERUP_BATTLESUIT:
	...
	case EV_POWERUP_REGEN:
		...
		if ( es->number == cg.snap->ps.clientNum ) {
			cg.powerupActive = RUNE_REGENERATION;
			cg.powerupTime = cg.time;
		}
	...
	// RQ3:  events
	case EV_RESISTANCE:
		DEBUGNAME("EV_RESISTANCE");
		trap_S_StartSound (NULL, es->number, CHAN_ITEM, cgs.media.protectSound);
		break;
	case EV_REGISTER_ITEM:	// we just swapped in an item that wasn't registered on the server before
		DEBUGNAME("EV_REGISTER_ITEM");
		CG_RegisterItemVisuals(es->eventParm);
		break;
	case EV_DYNAMIC_SOUND:
		DEBUGNAME("EV_DYNAMIC_SOUND");
		switch (es->eventParm)
		{
		case DYN_SOUND_MEGAHEALTH:
			trap_S_StartSound (position, es->number, CHAN_BODY,	trap_S_RegisterSound( "sound/items/m_health.wav", qfalse ) );
			break;
		case DYN_SOUND_ARMOR:
			trap_S_StartSound (position, es->number, CHAN_BODY,	trap_S_RegisterSound( "sound/misc/ar2_pkup.wav", qfalse ) );
			break;
		case DYN_SOUND_IMPACT:
			trap_S_StartSound (position, es->number, CHAN_BODY,	trap_S_RegisterSound( "sound/weapons/melee/fstatck.wav", qfalse ) );
			break;
		}
		break;
