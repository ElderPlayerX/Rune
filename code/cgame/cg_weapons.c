...
void CG_RegisterItemVisuals( int itemNum ) {
	...
	if ( item->giType == IT_POWERUP || item->giType == IT_HEALTH || 
		item->giType == IT_ARMOR || item->giType == IT_HOLDABLE || item->giType == IT_RUNE) {
...
//		VectorCopy(cg.refdef.vieworg, muzzlePoint );
	...
	{
		if ( cg_entities[trace.entityNum].currentState.powerups & ( 1 << PW_PHASE ) )
		{
...
static void CG_AddWeaponWithPowerups( refEntity_t *gun, entityState_t *state ) {
	...
	else if ( powerups & ( 1 << PW_PHASE ) ) 
	{
		gun->customShader = cgs.media.phaseShader;
		trap_R_AddRefEntityToScene( gun );
	} 
	else 
	{
		...
		if (state->generic1 & (1 << SFX_SHELL_QUAD))
		{
			gun->customShader = cgs.media.quadShader;
			trap_R_AddRefEntityToScene( gun );
		}

		if (state->generic1 & (1 << SFX_SHELL_BERSERK))
		{
			gun->customShader = cgs.media.berserkWeaponShader;
			trap_R_AddRefEntityToScene( gun );
		}

		if (state->generic1 & (1 << SFX_SHELL_SHIELD))
		{
			gun->customShader = cgs.media.battleWeaponShader;
			trap_R_AddRefEntityToScene( gun );
		}

		if (state->generic1 & (1 << SFX_SHELL_REFLECTION))
		{
			gun->customShader = cgs.media.reflectionShader;
			trap_R_AddRefEntityToScene( gun );
		}

		if (state->generic1 & (1 << SFX_SHELL_FROZEN))
		{
			gun->customShader = cgs.media.frozenShader;
			trap_R_AddRefEntityToScene( gun );
		}
//		if (state->generic1 & ( 1 << SFX_SHELL_REGEN ) ) {
...
void CG_FireWeapon( centity_t *cent ) {
	...
	if ( cent->currentState.powerups & ( 1 << PW_QUAD ) || (cent->currentState.generic1 & (1 << SFX_SOUND_QUAD))) {
...
static void CG_ShotgunPellet( vec3_t start, vec3_t end, int skipNum ) {
	...
	{
		if ( cg_entities[tr.entityNum].currentState.powerups & ( 1 << PW_PHASE ) )
		{
			if ((int)(crandom()*100)%100<RUNE_PHASE_AVOID_CHANCE)
			{
				CG_ShotgunPellet( start, end, cg_entities[tr.entityNum].currentState.number );
				return;
			}
		}
