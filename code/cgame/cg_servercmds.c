...
void CG_Rune_ApplyWeirdness (const char *weirdType)
{
	if (!Q_stricmp(weirdType,"twirly"))
		cg.twirlyTime = cg.time + RUNE_WEIRDNESS_DURATION;
	else if (!Q_stricmp(weirdType,"shroom"))
		cg.shroomTime = cg.time + RUNE_WEIRDNESS_DURATION;
	else if (!Q_stricmp(weirdType,"drunk"))
		cg.drunkTime = cg.time + RUNE_WEIRDNESS_DURATION;
	else if (!Q_stricmp(weirdType,"blind"))
		cg.flashTime = cg.time + RUNE_FLASH_DURATION + RUNE_FLASH_FADEIN;
	else if (!Q_stricmp(weirdType,"clear"))
	{
		cg.twirlyTime = 0;
		cg.shroomTime = 0;
		cg.drunkTime = 0;
...
static void CG_ServerCommand( void ) {
  ...
  if ( Q_stricmp (cmd, "remapShader") == 0 ) {
	...
	if ( !strcmp( cmd, "rune_dropinform" ) ) 
	{
		CG_Rune_DropInform(atoi(CG_Argv(1)));
		return;
	}

	if ( !strcmp( cmd, "weirdness" ) ) 
	{
		CG_Rune_ApplyWeirdness(CG_Argv(1));
		return;
	}
